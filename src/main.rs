...
async fn main() {
    ...
    let server_status_main = Arc::new(Mutex::new(ServerStatus::Busy));
    ...
    if (response == "h") {
        //Host
        let server_status_clone = Arc::clone(&server_status_main);
        let server_status_client = Arc::clone(&server_status_main);
        ...
        game_client(tcp_connection.unwrap(), player_profile);
